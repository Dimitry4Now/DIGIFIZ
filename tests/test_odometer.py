...
def test_parse_tolerates_junk():
    ...
    assert parse("") == (0, 0)
    assert parse("nothing useful") == (0, 0)
...
def test_round_trip(tmp_path):
    ...
    assert (odometer.odometer, odometer.trip) == (4321, 12)
...
def test_missing_file_starts_at_zero(tmp_path):
    ...
    assert (odometer.odometer, odometer.trip) == (0, 0)
